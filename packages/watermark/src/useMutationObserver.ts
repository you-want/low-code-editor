...
export function removeTrackedElement(element: TrackedElement) {
  beTrackedElements.value = beTrackedElements.value.filter(
    (item) => item.el !== element.el
  );
}
...
function observeRemovedNode(mutation: MutationRecord) {
  ...
  mutation.removedNodes.forEach((item) => {
    const trackedElement = beTrackedElements.value.find(
      (element) => element.el === item && element.type === ObserveType.REMOVE
    );
    if (trackedElement) {
