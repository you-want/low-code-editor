import { ref, unref, shallowRef, getCurrentInstance, onBeforeUnmount } from "vue"
// import type { Ref } from "vue";
import { throttle } from "lodash-es";
import type { Attr, Ref } from "./types";
import {
  stopProtectWatermark,
  setupProtectWatermark,
} from "./useProtectWatermark";
...
export function /* A function that returns an object with two methods: `setWatermark` and `clear`. */
useWatermark(
  ...
  function updateWatermark(attr: Attr) {
    ...
    el.style.background = styleConfig.background = `url(${createBase64(
      attr
    )}) left top repeat`;
  }
